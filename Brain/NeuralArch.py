...
class ActorNetwork(nn.Module):
    def __init__(self, state_size,HIDDEN1_UNITS,HIDDEN2_UNITS):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        out1 = t.tanh(self.steering(x)) * 0.5
...
class CriticNetwork(nn.Module):
    def __init__(self, state_size, action_size,HIDDEN1_UNITS,HIDDEN2_UNITS):
